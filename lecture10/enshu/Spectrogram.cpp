...
vector<complex<double>> calcSpectrogram(
    SoundWave& wave,
    int frame_size,
    int shift_size
) {
    int num_frame = (wave.size()-frame_size)/shift_size;
    vector<complex<double>> spectrogram;
    for(int k=0; k<num_frame; k++) {
        vector<complex<double>> x = getFrame(wave, k, frame_size, shift_size);
        vector<complex<double>> xw = multWindowFunc(x);
        vector<complex<double>> X = dft(xw);
        for(int i=0; i<X.size(); i++) {
            spectrogram.push_back(X[i]);
        }
    }
    return(spectrogram);
}
...
int main(void){
    ...
    vector<complex<double>> s = calcSpectrogram(wave, frame_size, shift_size);
